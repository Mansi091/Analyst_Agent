...
The executor now asks AIAuth for authorization before it runs generated Pandas code in the Docker sandbox.
...
If the passport is missing, invalid, denied, expired, revoked, or AIAuth is unavailable, the executor will not run the Pandas sandbox. Approval-required actions currently return a blocked tool result; connect the LangGraph resume flow to AIAuth's approval endpoint before enabling that mode for an end-to-end run.
